const PdfGenerator = (() => {
  const PAGE1 = {
    general: {
      ...
      firmaPacientePagina1: { x: 160, y: 74, width: 300, height: 42 }
    },
    toxina: {
      ...
      firmaPacientePagina1: { x: 160, y: 90, width: 300, height: 42 }
    }
  };

  const PAGE2 = {
    general: {
      firmaPaciente: { x: 82, y: 286, width: 216, height: 48 },
      rutPaciente: { x: 120, y: 330 },
      firmaDra: { x: 322, y: 286, width: 216, height: 48 }
    },
    toxina: {
      firmaPaciente: { x: 82, y: 160, width: 216, height: 48 },
      rutPaciente: { x: 120, y: 198 },
      firmaDra: { x: 322, y: 160, width: 216, height: 48 }
    }
  };
  ...
  async function generateConsentPdf(options) {
    ...
    if (options.signatureDataUrl) {
      ...
      const signatureScale = Math.min(signatureBox.width / signatureImage.width, signatureBox.height / signatureImage.height) * 1.06;
      ...
      if (page2) {
        ...
        const signatureScale2 = Math.min(signatureBox2.width / signatureImage.width, signatureBox2.height / signatureImage.height) * 1.04;
    ...
    if (page2) {
      ...
      drawImageInBox(page2, doctorSignatureImage, page2Coords.firmaDra, 1.08);
    }
